...
void Sample::WriteEvent()
{
    ...
    for (int i = 0; i < 1000; ++i)
    {
        ...
        if (!numHeadshotsStat.err())
        {
            intStat = numHeadshotsStat.payload().as_integer();
        }
        for (auto i = 0; i < 50; ++i)
        {
            ++intStat;
            performance_counters::get_singleton_instance()->begin_capture(L"set_stat_as_integer");
            m_statsManager->set_stat_as_integer(user, L"numHeadshots", intStat);
    ...
    str << performance_counters::get_singleton_instance()->get_capture_instace(L"get_stat")->average_time() << "/";
    ...
    str << L"max ";
    str << performance_counters::get_singleton_instance()->get_capture_instace(L"add_local_user")->max_time() << "/";
    str << performance_counters::get_singleton_instance()->get_capture_instace(L"get_stat")->max_time() << "/";
    ...
    auto strStr = str.str();
    OutputDebugString(strStr.c_str());
}
...
void Sample::Update(DX::StepTimer const& timer)
{
    ...
    {
        //auto events = m_statsManager->do_work();
        //for (auto evt : events)
        //{
